...
5. 로컬 네트워크 접근을 허용하고 프로필을 등록한 뒤 설정 → 연결 확인을 누릅니다.
6. 서버가 Gemini 모드인지 확인한 뒤 촬영/앨범 → 분석 결과 확인 → 저장으로 진행합니다. 기본 데모 모드에서는 예시 결과이며 저장할 수 없습니다.
...
현재는 **같은 Wi-Fi에서 Expo Go로 먼저 사용**하는 단계입니다. 위 3-B 설정 후 Mac의 Java 서버와 Expo 개발 서버를 실행한 채 QR을 스캔합니다. 외부 공개나 공유기 포트 포워딩은 필요하지 않습니다. 연결 후 실제 촬영·앨범 HEIC·권한 거부·저장·재실행·서버 단절을 확인합니다. 웹 미리보기와 아이폰의 기록은 서로 별개입니다.

Mac을 끄거나 외출 중에도 AI를 쓰려면 두 가지 준비가 필요합니다.

1. 인증·사용자별 호출 제한을 추가한 Java 서버를 HTTPS로 운영합니다. 현재 인증 없는 로컬 서버를 그대로 외부 공개하지 않습니다.
2. API 주소를 운영 서버로 지정한 iOS 앱을 빌드·서명해 설치합니다. TestFlight 배포에는 Apple Developer Program 및 App Store Connect 준비가 필요합니다. 아직 배포 설정·독립 빌드는 만들지 않았습니다.

- [Expo 개발 서버와 QR 실행](https://docs.expo.dev/get-started/start-developing/)
- [Expo 프로덕션 빌드·TestFlight 경로](https://docs.expo.dev/deploy/build-project/)

2026-09-19 점검 시 연결된 실제 아이폰은 없었습니다. Simulator에서 성공한 결과를 실기기 검증으로 대신하지 않습니다. 매일 기록을 쌓기 전에 내보내기·복원 기능을 추가하는 것을 권장합니다.
