...
| 자주 하는 행동 | 기록/진행 확인 화면 중심으로 반복 접근 | Today·기록·통계·Settings 하단 고정, 촬영 한 번 접근, 결과 안내는 필요한 만큼 펼치기 |
...
- 이름 기록: 직접 기록 → 음식 이름·선택적인 양 → 예상 칼로리 채우기 → 가정 확인·수정 → 저장. 칼로리 입력을 첫 필수 항목으로 요구하지 않음.
- 통계: 월/연 전환 → 기록일 평균·총합·기록률 → 월 또는 날짜 선택 → 식사 확인. 미기록을 0으로 취급하지 않음.
- 추천: 조건에 따른 Gemini 새 메뉴 → 탄단지·재료·조리법 확인. 다른 메뉴 재생성, 실패 시 기본 레시피 표시.
...
실사용자에게 처음 목표 설정과 한 끼 기록을 맡겨 완료 시간, 길을 잃는 지점, 취소·오입력 빈도를 관찰해야 합니다. 2026-09-19 기준 맞춤 AI 레시피와 추천 탄단지, 이름으로 기록, 월·연 통계를 구현했습니다. 백업·계정 동기화, 저장된 식사의 영양 DB 매칭·탄단지, 기록 검색은 아직 없습니다.
