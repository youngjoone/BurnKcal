...
export function JournalScreen({
  ...
  const visible = meals.filter(
    (m) => !m.deletedAt && (all || m.localDate === day),
  );
  return (
    <>
      <View style={styles.stack}>
        <Text style={styles.title}>식사 기록</Text>
        <Text style={styles.subtitle}>확인하고 저장한 식사만 합산해요.</Text>
      </View>
      <View style={styles.card}>
        <Text style={styles.small}>오늘 섭취량 · {day}</Text>
      ...
      {!visible.length && (
        <View style={styles.card}>
          <Text style={styles.label}>아직 저장한 식사가 없어요.</Text>
      ...
      <Button title="새 식사 분석" disabled={busy} onPress={onScan} />
