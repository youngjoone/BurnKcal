...
**사진·음식 이름 분석과 맞춤 레시피 추천에 Gemini를 연결했습니다.** 기본 실행은 데모이며 서버의 `.env`에서 Gemini 모드를 설정하면 실제 분석을 사용합니다.
...
- 음식 이름만 입력 → Gemini로 1인분 예상 칼로리·양 가정 확인 → 수정·저장. 칼로리를 아는 경우 직접 입력도 가능
- 월별 달력에서 날짜별 기록·총합 확인. 지난 날짜에 사진 또는 직접 입력으로 식사 추가
- 식사 수정·삭제·복구, 중복 저장 방지, 앱 재실행 후 기록 유지
- 월별·연도별 총 칼로리·기록일 평균·식사 수·기록한 날, 날짜별 기록으로 이동
- 목표·남은 칼로리·제외 재료·조리 시간에 맞춘 Gemini 레시피와 다른 메뉴 다시 추천
- 추천의 칼로리·단백질·탄수화물·지방, 재료량·조리 순서
- iPhone SQLite 저장, 웹 미리보기는 별도 localStorage 사용
- 사진 JPEG 정규화·크기 조절, 입력 검증, 로딩·오류·재시도

Gemini가 새 레시피를 생성하며 실패·로딩 때는 기본 레시피 6종을 대체안으로 표시합니다. 기본 레시피 영양값은 USDA 자료와 재료 중량으로 계산합니다. AI 분석과 하루 목표는 추정값이며, 매우 적은 잔여 칼로리에 끼니를 억지로 맞추지 않습니다. 데모 분석은 실제 식사로 저장할 수 없습니다.

로그인·백업·기기 간 동기화·사진 영구 보관·저장한 식사의 탄단지 분석·서버 공개 배포는 아직 없습니다. 현재는 Mac 개발 서버와 Expo Go를 이용하는 개발용 앱입니다.
...
| [STATISTICS](docs/STATISTICS.md) | 월·연 평균과 미기록일은 어떻게 집계하는가? |
| [RECIPE_NUTRITION](docs/RECIPE_NUTRITION.md) | 추천 영양값의 출처와 계산 기준은 무엇인가? |
